...
def knockPort(host, port):
    try:
        ...
        banner = skt.recv(128).decode('utf-8', 'ignore').strip('\n').strip('\r')
        geoLoc = getGeoLoc(host)
        ## Print Response
        thread_lock.acquire()
        scan_list.append('\n%s : %s : %s : %s' % (host, port, banner, geoLoc))
        stdout.write('\n[+] %s#%s : %s : %s' % (host, port, banner, geoLoc))
    except:
        pass
    finally:
...
def parsePortRanges(toParse):
    ...
    for port in parseMe:
        ...
        if port == '*':
            # Use common ports
            ports = [20,21,22,23,25,43,53,80,110,143,443,464,1080,1194,1900,4444,9988]
        elif '-' in port:
            # Use user selected ports
            p_range = port.split('-')
...
def main():
    ...
    stdout.write('\n||---------------------||\n\n')
    user_octets = input('Hosts2Scan [1-255]: ')
    user_ports = input('Ports2Scan [1-65535]: ')
    user_listen_timeout = float(input('Listen Timeout: '))
    user_thread_timeout = float(input('Thread Timeout: '))
    user_log = input('Log Results [y, n]: ')
